					...
					<div class="the-box full no-border">
						<div class="table-responsive">
							<table class="table table-th-block table-danger">
								<thead>
									<tr><th style="width: 85px;">Pack No.</th><th>Package Type</th><th>Price per Room</th><th>Active</th></tr>
								</thead>
								<tbody>
								<?PHP
								$sql="SELECT * FROM packages INNER JOIN room_price ON packages.pack_type=room_price.room_type";
								$result=mysql_query($sql);
								if (mysql_num_rows($result) > 0) {	
									while ($row = mysql_fetch_array($result)) {	
										...
										echo'<tr>
												<td>'.$pack_no.'</td>												
												<td><a href="edit_advertise.php?pack_no='.$pack_no.'">'.$pack_type.'</a></td>
												<td>'.$price.'</td>';
												if ($active=='Yes'){
